...
pub const ClosureExpr = struct { params: []const Field, return_type: NodeIndex, body: NodeIndex, is_sendable: bool = false, is_async: bool = false, span: Span };
...
pub const Ast = struct {
    ...
    /// Check if the file uses async features (async functions, async closures, or await).
    /// Scans all nodes in the AST at any nesting level.
    /// Used by driver to auto-import cooperative executor on Wasm.
