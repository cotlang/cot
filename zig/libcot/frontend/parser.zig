...
pub const Parser = struct {
    ...
    fn parseDecl(self: *Parser) ParseError!?NodeIndex {
        return switch (self.tok.tok) {
            ...
            .kw_fn => self.parseFnDecl(false, false, false, false, false),
            .kw_static => blk: {
                self.advance();
    ...
    fn parseExternFn(self: *Parser) ParseError!?NodeIndex {
        ...
        return self.parseFnDecl(true, false, false, false, false);
    }
    ...
    fn parseExportFn(self: *Parser) ParseError!?NodeIndex {
        ...
        return self.parseFnDecl(false, false, false, true, false);
    }
    ...
    fn parseAsyncFn(self: *Parser) ParseError!?NodeIndex {
        ...
        // Peek past 'fn' to distinguish async closure from async function decl.
        // async fn(params) { body } — async closure (Swift: @Sendable () async -> T)
        // async fn name(params) { body } — named async function declaration
        const after_fn = self.peekToken();
        if (after_fn.tok == .lparen) {
            // Async closure: consume 'fn', then parse closure body.
        ...
        // Named async function declaration: parseFnDecl consumes 'fn'.
    ...
    fn parseStructDeclWithLayout(self: *Parser, layout: ast.StructLayout) ParseError!?NodeIndex {
        ...
        while (!self.check(.rbrace) and !self.check(.eof)) {
            ...
            if (self.check(.kw_const) or self.check(.kw_enum) or self.check(.kw_type) or
                self.check(.kw_fn) or self.check(.kw_static) or self.check(.at) or
    ...
    fn parseImplBlock(self: *Parser) ParseError!?NodeIndex {
        ...
        while (!self.check(.rbrace) and !self.check(.eof)) {
            ...
            if (self.check(.kw_static)) {
                ...
                if (try self.parseFnDecl(false, false, true, false, false)) |idx| try methods.append(self.allocator, idx);
            } else if (self.check(.at) and self.peekToken().tok == .ident and std.mem.eql(u8, self.peekToken().text, "inlinable")) {
                self.advance(); // @
                self.advance(); // inlinable
                if (!self.check(.kw_fn)) { self.syntaxError("expected 'fn' after '@inlinable'"); self.advance(); continue; }
                if (try self.parseFnDecl(false, false, false, false, true)) |idx| try methods.append(self.allocator, idx);
    ...
    fn parseOperand(self: *Parser) ParseError!?NodeIndex {
        ...
        switch (self.tok.tok) {
            ...
            .kw_async => {
                ...
                // Swift reference: @Sendable () async throws -> T
                self.advance(); // consume 'async'
                if (!self.check(.kw_fn)) { self.syntaxError("expected 'fn' after 'async'"); return null; }
                self.advance(); // consume 'fn'
                if (!self.expect(.lparen)) return null;
                const async_params = try self.parseFieldList(.rparen);
                if (!self.expect(.rparen)) return null;
                var async_return_type: NodeIndex = null_node;
                if (!self.check(.lbrace) and !self.check(.eof))
                    async_return_type = try self.parseType() orelse null_node;
                const async_body = try self.parseBlock() orelse return null;
                return try self.tree.addExpr(.{ .closure_expr = .{
                    .params = async_params,
                    .return_type = async_return_type,
                    .body = async_body,
                    .is_async = true,
                    .span = Span.init(start, self.pos()),
                } });
