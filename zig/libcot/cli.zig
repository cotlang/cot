...
pub const NativeBackend = enum {
    cranelift, // rust/libclif — real Cranelift (default)
    zig_clif, // zig/libclif — hand-ported Cranelift
};
...
pub const BuildOptions = struct {
    ...
    lib: bool = false,
    ssa: ?[]const u8 = null,
    /// Native backend selection. Default: cranelift (rust/libclif).
    /// --backend=zig uses the hand-ported Zig Cranelift backend.
    backend: NativeBackend = .cranelift,
    /// Swift -strict-concurrency=complete equivalent.
    /// Cot defaults to complete (strict from day one). This flag is explicit confirmation.
    /// Swift reference: -strict-concurrency=minimal|targeted|complete
...
fn parseBuild(args: *std.process.ArgIterator) ?Command {
    ...
    while (args.next()) |arg| {
        ...
        } else if (std.mem.eql(u8, arg, "--backend=zig")) {
            opts.backend = .zig_clif;
        } else if (std.mem.eql(u8, arg, "--backend=cranelift") or std.mem.eql(u8, arg, "--backend=rust")) {
            opts.backend = .cranelift;
        } else if (std.mem.eql(u8, arg, "--strict-concurrency") or
            std.mem.eql(u8, arg, "--strict-concurrency=complete"))
        {
            // Swift: -strict-concurrency=complete
            // Cot defaults to complete (strict from day one). Flag is explicit confirmation.
            opts.strict_concurrency = true;
        } else if (std.mem.eql(u8, arg, "--strict-concurrency=minimal") or
            std.mem.eql(u8, arg, "--strict-concurrency=targeted"))
        {
            // Swift: minimal/targeted modes relax Sendable checking.
            // Cot does not support relaxed modes — always strict.
            std.debug.print("Warning: Cot only supports --strict-concurrency=complete (default)\n", .{});
...
fn printBuildHelp() void {
    std.debug.print(
        ...
        \\  --strict-concurrency  Strict Sendable/isolation checking (default: complete)
        \\
        \\Examples:
