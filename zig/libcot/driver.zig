...
const ts_transform = @import("ts_transform.zig");
...
/// Delegates to libts.transform which owns all the TS→Cot mapping logic.
fn transformTsFile(ts_tree: *const libts.ts_ast.Ast, cot_tree: *ast_mod.Ast, allocator: std.mem.Allocator, filename: []const u8) error{OutOfMemory}!void {
    return ts_transform.transformFile(ts_tree, cot_tree, allocator, filename);
}
