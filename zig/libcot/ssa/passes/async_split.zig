...
pub fn asyncSplit(f: *Func, type_registry: *const TypeRegistry) !void {
    ...
    // Only transform actual poll functions (constructor/poll split pattern).
    // Non-poll functions with 2+ awaits use eager evaluation (each await blocks sequentially).
    // Poll functions are generated by lowerAsyncConstructorPoll and named with __poll suffix.
...
fn splitBlockAtSuspend(f: *Func, sp: *SuspendPoint, local_to_offset: *const std.AutoHashMapUnmanaged(u32, i64), state_offset: i64, call_result_offset: i64) !void {
    ...
    // Must update predecessor edges in successor blocks to point to resume_block
    resume_block.succs = block.succs;
    for (resume_block.succs) |edge| {
        // Update pred entry in successor block: replace block → resume_block
    ...
    // Return yield sentinel (0 = not done, function will be polled again)
    // The poll function returns i64: 0 = yielded, nonzero = done with result
    {
