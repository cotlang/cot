        ...
        switch (ssa_blk.kind) {
            .ret => {
                ...
                if (ssa_blk.controls[0]) |ctrl| {
                    debug.log(.codegen, "ssa_to_clif: RET ctrl.op={s} ctrl.id={d} sig_returns={d}", .{ @tagName(ctrl.op), ctrl.id, self.clif_func.signature.returns.items.len });
                    if ((ctrl.op == .string_make or ctrl.op == .slice_make or ctrl.op == .opt_make) and ctrl.args.len >= 2) {
                        ...
                        // The ptr is in value_map[ctrl.id], the len is in compound_extra_map[ctrl.id]
                        // (set by the string_make/slice_make/opt_make handler)
                        const ptr = self.getClif(ctrl);
                        const has_compound = self.compound_extra_map.get(ctrl.id);
                        debug.log(.codegen, "ssa_to_clif: string_make return ctrl.id={d} compound={}", .{ ctrl.id, has_compound != null });
                        const len = has_compound orelse self.getClif(ctrl.args[1]);
                        ...
                        const ptr = self.getClif(ctrl);
                        debug.log(.codegen, "ssa_to_clif: compound return ctrl.op={s} ctrl.id={d}", .{ @tagName(ctrl.op), ctrl.id });
                        _ = try ins.return_(&[_]clif.Value{ ptr, len_val });
                    } else if (self.clif_func.signature.returns.items.len >= 2) {
                        // String/slice return: function signature has 2 returns (ptr, len).
                        const ptr = self.getClif(ctrl);
                        debug.log(.codegen, "ssa_to_clif: string return ctrl.op={s} ctrl.id={d}", .{ @tagName(ctrl.op), ctrl.id });
                        // Try select1 sibling: if ctrl is select0, the next value is select1 with the len
                        if (ctrl.op == .select0 and ctrl.args.len > 0) {
                            // The call result's second component — find select1 for the same call
                            const call_val = ctrl.args[0];
                            if (self.compound_extra_map.get(call_val.id)) |len| {
                                _ = try ins.return_(&[_]clif.Value{ ptr, len });
                            } else {
                                // Fall back: return ptr with 0 len (will cause test failure but won't crash)
                                const zero = try ins.iconst(clif.Type.I64, 0);
                                _ = try ins.return_(&[_]clif.Value{ ptr, zero });
                            }
                        } else {
                            // String from direct load — both ptr and len should be in value_map
                            // if the string was properly decomposed by the SSA passes
                            const zero = try ins.iconst(clif.Type.I64, 0);
                            _ = try ins.return_(&[_]clif.Value{ ptr, zero });
                        }
                    } else {
