...
> **SUPERSEDED:** This document contains outdated theories. See `claude/SELFCOT_MEMORY_AUDIT.md` for the current analysis. Key correction: the "deinit fix" described below was **never committed** — List and Map still have no `deinit` method. The root cause is three independent issues: ARC gap (couldBeARC returns false for collections), no arena allocator, and value-copy overhead.
...
5. Go Cache pattern attempted — encountered SIGBUS crash (SsaValue struct was restructured, broke runtime)

## Memory Leak Fix Applied (2026-03-15)

### Root Cause: List/Map backing arrays not freed at scope exit

`List(T)` and `Map(K,V)` store backing arrays as `items: i64` (raw pointer cast to int). ARC only tracks managed pointers (strings), so when a List/Map goes out of scope, ARC does nothing — the backing arrays leak.

The Zig compiler has a `scope_destroy` mechanism (`compiler/frontend/lower.zig:2017`) that auto-detects `deinit()` methods on structs and inserts cleanup calls at scope exit. But List/Map only had `free()` — the compiler only triggers scope_destroy for `deinit`, not `free`.

### Fix

Added `fn deinit` to both `List(T)` (stdlib/list.cot) and `Map(K,V)` (stdlib/map.cot) that calls `self.free()`.

### Proof

Stress test: 100K Maps + 100K Lists created in a loop:
- **Without deinit: 167MB RSS** (backing arrays leaked every iteration)
- **With deinit: 48MB RSS** (backing arrays freed at scope exit)

### How reference compilers manage this

**Go**: `defer f.Cache.freeSlice()` per pass + `Cache.Reset()` between functions. Pooled allocators in `cache.go`/`allocators.go`.

**Zig Cot compiler**: `ArenaAllocator` per function — `defer func_arena.deinit()` frees ALL SSA temps at once (`driver.zig:5712-5715`). Why it uses 64MB.

## Current Blocker: selfcot Crashes (SIGSEGV)

Cannot verify memory fix end-to-end — selfcot crashes before reaching codegen.

### How to build/test selfcot:
```bash
zig build                                                                 # Build Zig compiler
cot build self/main.cot -o /tmp/selfcot                                   # Build selfcot
/usr/bin/time -l /tmp/selfcot build self/frontend/scanner.cot -o /tmp/out  # Measure
```

### Crash backtrace:
```
EXC_BAD_ACCESS (code=1, address=0x20)
  Map(string,int)_contains + 48      ← null pointer dereference (capacity at offset 32)
  Checker_isDefinedInSameFile + 116  ← checker.cot:418
  Checker_collectFnDecl + 1396
  Checker_checkFile + 384
  main.compileSingleFile + 884
```

### Diagnosis:
Checker struct (~360 bytes, 28 fields) is returned by value from `Checker.init()`. During struct copy, `scope_maps` field (heap pointer from `allocMap()`) becomes 0. `getScopeMap(0)` returns `@intToPtr(*Map, 0)` → `Map.contains` loads from address 0x20 → SIGSEGV.

**This is a native codegen bug** in the Cranelift backend for large struct returns. Fix in `compiler/codegen/native/ssa_to_clif.zig`.
