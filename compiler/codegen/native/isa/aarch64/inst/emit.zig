...
/// Emit a load_addr for an arbitrary offset from a base register.
/// Port of Cranelift's LoadAddr emit (emit.rs:3316-3395).
/// For small offsets, emits a single ADD/SUB. For large offsets, emits
/// MOVZ/MOVK to load the offset into spilltmp (x16), then ADD rd, base, x16.
fn emitLoadAddr(sink: *MachBuffer, rd: Writable(Reg), basereg: Reg, off: i64) !void {
    if (off == 0) {
        // MOV rd, basereg (ORR rd, xzr, basereg)
        try sink.put4(encArithRrr(0b10101010_000, 0b000_000, rd, zeroReg(), basereg));
        return;
    }

    const abs_off: u64 = if (off >= 0) @intCast(off) else @intCast(-off);

    // Try Imm12 encoding (0-4095 unshifted, or 0-16773120 shifted by 12)
    if (Imm12.maybeFromU64(abs_off)) |imm12| {
        if (off >= 0) {
            // ADD rd, basereg, #imm12
            try sink.put4(encArithRrImm12(0b100_10001, imm12.shiftBits(), imm12.immBits(), basereg, rd));
        } else {
            // SUB rd, basereg, #imm12
            try sink.put4(encArithRrImm12(0b110_10001, imm12.shiftBits(), imm12.immBits(), basereg, rd));
        }
        return;
    }

    // Large offset: MOVZ/MOVK into spilltmp, then ADD/SUB rd, basereg, spilltmp.
    // Port of Cranelift's large-offset path using load_constant + ADD.
    const tmp = spilltmpReg();
    const tmp_wr = Writable(Reg).fromReg(tmp);

    // Emit MOVZ for first halfword, MOVK for subsequent non-zero halfwords
    ...
    while (remaining != 0 or first) : (shift += 16) {
        ...
        if (hw != 0 or first) {
            if (first) {
                try sink.put4(encMoveWide(.movz, tmp_wr, MoveWideConst{ .bits_val = hw, .shift = shift / 16 }, .size64));
                first = false;
            } else {
                try sink.put4(encMovk(tmp_wr, MoveWideConst{ .bits_val = hw, .shift = shift / 16 }, .size64));
            }
        }
        if (shift >= 48) break;
    }
...
/// Emit pre-instructions generated by memFinalize (e.g., MOVZ/MOVK for large offsets).
            ...
            switch (payload.mem) {
                .reg_offset => |m| {
                    if (m.offset == 0) {
                        ...
                        // Large offset: MOVZ/MOVK into spilltmp, then ADD/SUB
                        try emitLoadAddr(sink, payload.rd, m.rn, m.offset);
                    }
                },
                .sp_offset => |m| {
                    if (m.offset >= 0 and m.offset < 4096) {
                        ...
                        try emitLoadAddr(sink, payload.rd, stackReg(), m.offset);
                    }
                },
                .slot_offset => |m| {
                    ...
                    if (off >= 0 and off < 4096) {
                        ...
                        try emitLoadAddr(sink, payload.rd, stackReg(), off);
                    }
                },
                .fp_offset => |m| {
                    if (m.offset >= 0 and m.offset < 4096) {
                        ...
                        try emitLoadAddr(sink, payload.rd, fpReg(), m.offset);
                    }
                },
