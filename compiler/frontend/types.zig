...
// ============================================================================
// Shape: Go-style GC shape stenciling for generic deduplication
// ============================================================================
// Reference: Go's shapify() in cmd/compile/internal/noder/reader.go:891-969.
// A shape describes a type's memory footprint and ARC behavior. Two types with
// the same shape produce identical machine code for layout-only operations
// (load, store, copy, pass, ARC retain/release).

pub const Shape = struct {
    size: u32,
    alignment: u32,
    arc_kind: ArcKind,
    /// Register class: int and float values use different register files on ARM64/x64.
    /// Without this, i64 and f64 (both 8 bytes, arc=none) would share a stencil,
    /// causing register file mismatches at call boundaries.
    /// Reference: Go shapify() distinguishes int/float shapes for this exact reason.
    reg_class: RegClass,

    pub const ArcKind = enum(u8) {
        /// Value types: i8, u8, i16, u16, i32, u32, f32, i64, u64, f64, bool
        none,
        /// All *T: uniform ARC via header (retain/release same for all T)
        pointer,
        /// All []T, string: ARC on backing buffer
        slice,
    };

    /// Register class for value types. Determines which physical registers are used.
    pub const RegClass = enum(u8) {
        int, // i8-i64, u8-u64, bool, enum, pointer — GPR (x0-x7 / rax-r15)
        float, // f32, f64 — FPR (d0-d7 / xmm0-xmm15)
        compound, // structs, slices, etc. — passed in memory or multi-reg
    };

    /// Compute the shape for a concrete type.
    pub fn fromType(reg: *const TypeRegistry, ty: TypeIndex) Shape {
        const t = reg.get(ty);
        // All pointers share the same ARC shape (header-based retain/release)
        if (t == .pointer) return .{ .size = 8, .alignment = 8, .arc_kind = .pointer, .reg_class = .int };
        // All slices (including string = []u8) share the same shape
        if (t == .slice) return .{ .size = 24, .alignment = 8, .arc_kind = .slice, .reg_class = .compound };
        // Primitives and enums: value types grouped by size AND register class
        if (t == .basic or t == .enum_type) {
            const size = reg.sizeOf(ty);
            const rc: RegClass = if (t == .basic and (t.basic == .f32_type or t.basic == .f64_type))
                .float
            else
                .int;
            return .{ .size = size, .alignment = if (size == 0) 1 else @min(size, 8), .arc_kind = .none, .reg_class = rc };
        }
        // Everything else (structs, optionals, error_unions, tuples, etc.):
        // return their actual layout but never stencilable (canShapeStencil will reject)
        const size = reg.sizeOf(ty);
        return .{ .size = size, .alignment = if (size == 0) 1 else @min(size, 8), .arc_kind = .none, .reg_class = .compound };
    }

    pub fn eql(a: Shape, b: Shape) bool {
        return a.size == b.size and a.alignment == b.alignment and a.arc_kind == b.arc_kind and a.reg_class == b.reg_class;
    }

    /// Returns shape suffix for function naming: "s8in", "s8fn", "s4ip", etc.
    /// Format: "s{size}{reg_class_char}{arc_char}"
    pub fn key(self: Shape) ShapeKey {
        var sk: ShapeKey = .{};
        const arc_char: u8 = switch (self.arc_kind) {
            .none => 'n',
            .pointer => 'p',
            .slice => 's',
        };
        const rc_char: u8 = switch (self.reg_class) {
            .int => 'i',
            .float => 'f',
            .compound => 'c',
        };
        const result = std.fmt.bufPrint(&sk.buf, "s{d}{c}{c}", .{ self.size, rc_char, arc_char }) catch unreachable;
        sk.len = @intCast(result.len);
        return sk;
    }
    ...
    pub const ShapeKey = struct {
        ...
        pub fn slice(self: *const ShapeKey) []const u8 {
...
test "Shape.fromType basic types" {
    var reg = try TypeRegistry.init(std.testing.allocator);
    defer reg.deinit();

    // Same-size, same-register-class types share shape
    const s_i64 = Shape.fromType(&reg, TypeRegistry.I64);
    const s_u64 = Shape.fromType(&reg, TypeRegistry.U64);
    const s_f64 = Shape.fromType(&reg, TypeRegistry.F64);
    try std.testing.expect(s_i64.eql(s_u64));
    // f64 and i64 have different register classes — must NOT share shape
    try std.testing.expect(!s_i64.eql(s_f64));
    try std.testing.expectEqual(@as(u32, 8), s_i64.size);
    try std.testing.expectEqual(Shape.ArcKind.none, s_i64.arc_kind);

    // Different-size types have different shapes
    const s_i32 = Shape.fromType(&reg, TypeRegistry.I32);
    try std.testing.expect(!s_i64.eql(s_i32));
    try std.testing.expectEqual(@as(u32, 4), s_i32.size);

    // Bool is 1 byte
    const s_bool = Shape.fromType(&reg, TypeRegistry.BOOL);
    try std.testing.expectEqual(@as(u32, 1), s_bool.size);
}

test "Shape.fromType pointers and slices" {
    var reg = try TypeRegistry.init(std.testing.allocator);
    defer reg.deinit();

    // All pointer types share the same shape
    const my_struct = try reg.add(.{ .struct_type = .{ .name = "TestA", .fields = &.{}, .size = 16, .alignment = 8 } });
    const my_struct2 = try reg.add(.{ .struct_type = .{ .name = "TestB", .fields = &.{}, .size = 24, .alignment = 8 } });
    const ptr_a = try reg.makePointer(my_struct);
    const ptr_b = try reg.makePointer(my_struct2);
    const s_pa = Shape.fromType(&reg, ptr_a);
    const s_pb = Shape.fromType(&reg, ptr_b);
    try std.testing.expect(s_pa.eql(s_pb));
    try std.testing.expectEqual(Shape.ArcKind.pointer, s_pa.arc_kind);

    // String and slices share the same shape
    const s_str = Shape.fromType(&reg, TypeRegistry.STRING);
    const slice_i32 = try reg.makeSlice(TypeRegistry.I32);
    const s_slice = Shape.fromType(&reg, slice_i32);
    try std.testing.expect(s_str.eql(s_slice));
    try std.testing.expectEqual(Shape.ArcKind.slice, s_str.arc_kind);
    try std.testing.expectEqual(@as(u32, 24), s_str.size);
}

test "Shape.key format" {
    const s = Shape{ .size = 8, .alignment = 8, .arc_kind = .pointer, .reg_class = .int };
    const k = s.key();
    try std.testing.expectEqualStrings("s8ip", k.slice());

    const s2 = Shape{ .size = 4, .alignment = 4, .arc_kind = .none, .reg_class = .int };
    const k2 = s2.key();
    try std.testing.expectEqualStrings("s4in", k2.slice());

    const s3 = Shape{ .size = 24, .alignment = 8, .arc_kind = .slice, .reg_class = .compound };
    const k3 = s3.key();
    try std.testing.expectEqualStrings("s24cs", k3.slice());

    // Float shapes must differ from int shapes of same size
    const s4 = Shape{ .size = 8, .alignment = 8, .arc_kind = .none, .reg_class = .float };
    const k4 = s4.key();
    try std.testing.expectEqualStrings("s8fn", k4.slice());

    const s5 = Shape{ .size = 8, .alignment = 8, .arc_kind = .none, .reg_class = .int };
    const k5 = s5.key();
    try std.testing.expectEqualStrings("s8in", k5.slice());
    try std.testing.expect(!std.mem.eql(u8, k4.slice(), k5.slice()));
}
