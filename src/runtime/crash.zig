...
//! - Terminal reset for raw mode recovery
//! - Stack trace printing with symbol resolution
//! - VM crash context integration
//! - Hooks for modules (TUI, ISAM) to register cleanup callbacks
...
/// Print stack trace with proper line endings for raw terminal mode
...
fn crashSignalHandler(sig: c_int) callconv(.c) void {
    // FIRST: Reset terminal using escape codes (async-signal-safe)
    // This ensures output is readable even if TUI was in raw mode
    const reset_seq = "\x1b[?1049l" ++ // Exit alternate screen buffer
        "\x1b[?25h" ++ // Show cursor
        "\x1bc"; // Full terminal reset (RIS)
    _ = std.posix.write(2, reset_seq) catch {};
