...
pub const Evaluator = struct {
    ...
    pub fn evaluateExpression(self: *Self, expr_idx: ExprIdx) EvalError!Value {
        ...
        return switch (tag) {
            ...
            .call, .method_call, .index, .member, .optional_member, .optional_index, .range, .array_init, .struct_init, .generic_struct_init, .lambda, .if_expr, .match_expr, .block_expr, .is_expr, .interp_string => EvalError.InvalidExpression,
        };
    }
