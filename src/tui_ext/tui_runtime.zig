...
/// Signal handler that restores terminal before crash
fn crashSignalHandler(sig: c_int) callconv(.c) void {
    // Restore terminal to normal mode before crash output
    if (global_tui) |*state| {
    ...
    std.posix.sigaction(std.posix.SIG.SEGV, &default_action, null);
    std.posix.sigaction(std.posix.SIG.ABRT, &default_action, null);
    _ = std.c.raise(sig);
}

/// Install signal handlers for crash cleanup
fn installCrashHandlers() void {
    ...
    std.posix.sigaction(std.posix.SIG.SEGV, &action, null);
    std.posix.sigaction(std.posix.SIG.ABRT, &action, null);
}
...
pub fn devLog(message: []const u8) void {
    if (getState()) |state| {
        ...
        // Store the message in circular buffer
        // Note: We're storing slices, so caller must ensure they remain valid
        // For VM use, these are allocated strings
        const idx = (state.dev_log_start + state.dev_log_count) % 64;
        if (state.dev_log_count < 64) {
            state.dev_log_lines[idx] = message;
