//! Value Witness Table generation.
//!
//! Generates VWT witness functions for each concrete type. Each witness is a
//! real callable IR function that goes through the SSA pipeline to native/wasm.
//!
//! Architecture:
...
const Type = foundation.types.Type;
const ValueWitnessFlags = foundation.types.ValueWitnessFlags;
const ValueWitnessTable = foundation.types.ValueWitnessTable;
const Span = foundation.source.Span;
...
pub const VWTGenerator = struct {
    ...
    /// Metadata for a single type's VWT witness functions.
    pub const VWTEntry = struct {
        type_idx: TypeIndex,
        destroy_fn: []const u8,
        initializeWithCopy_fn: []const u8,
    ...
    /// All Cot types are bitwise-takable (no weak refs yet affect layout).
    ...
    /// Pointers have extra inhabitants (null). Integers have 0.
    ...
    fn classifyType(self: *VWTGenerator, type_idx: TypeIndex) TypeCategory {
        ...
        if (info == .pointer and info.pointer.flags.is_managed) return .managed_ptr;

        if (info == .list or info == .map) return .collection;
        if (info == .struct_type and TypeRegistry.isGenericCollection(info.struct_type.name)) return .collection;
        ...
        switch (cat) {
            ...
            .union_with_arc => {
                for (info.union_type.variants) |variant| {
                    if (variant.payload_type != .invalid and !self.type_reg.isTrivial(variant.payload_type)) {
    ...
    ///
    /// TypeMetadata layout (40 bytes):
    ///   [0]  vwt pointer                       (ptr to VWT global, 8B)
    ///   [1]  size                              (u64, 8B)
    ///   [2]  stride                            (u64, 8B)
    ///   [3]  kind                              (TypeIndex, 8B)
    ///   [4]  hash_fn_ptr                       (fn ptr, 8B)
    fn emitMetadataGlobals(self: *VWTGenerator, builder: *ir.Builder, entry: VWTEntry, type_name: []const u8, type_idx: TypeIndex, info: Type) !void {
        ...
        // TypeMetadata: [0]=vwt_ptr, [1]=size, [2]=stride, [3]=kind, [4]=hash_fn_ptr
        ...
        const kind_const = try fb.emitConstInt(@intCast(@intFromEnum(type_idx)), TypeRegistry.I64, Span.zero);
        _ = try fb.emitPtrStoreValue(meta_kind_addr, kind_const, Span.zero);

        // [4] hash_fn_ptr — for Hashable types, store their hash function address.
    ...
    /// Struct: field-by-field destroy in reverse order.
    ...
    fn emitTagSwitchDestroy(self: *VWTGenerator, fb: *ir.FuncBuilder, type_idx: TypeIndex) !void {
        ...
        for (variants, 0..) |variant, vi| {
            if (variant.payload_type == .invalid or self.type_reg.isTrivial(variant.payload_type)) continue;
    ...
    /// POD: memcpy. Managed ptr: memcpy + retain. Collection: memcpy + retain(buf).
    ...
    ///
    /// Destroy old dest value, then initializeWithCopy from src.
    ...
    ///
    /// Move src value into uninitialized dest. Source is invalidated.
    ...
    ///
    /// Destroy old dest value, then memcpy from src (take = move, no retain).
    ...
    ///
    /// Reads the discriminator tag from the union at offset 0.
    ...
    ///
    /// Returns a pointer to the payload area at offset 8.
    ...
    ///
    /// Writes a tag into the union at offset 0.
    ...
    ///
    /// For single-payload enums (e.g., Optional).
    ...
    ///
    /// Store tag for single-payload enum. whichCase 0 = payload, > 0 = empty case.
    ...
    ///
    /// Copy value from one existential buffer to another.
    /// If inline: memcpy(dest, src, 24). If out-of-line: copy the box pointer.
...
test "VWTGenerator classifyType categories" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    var reg = try TypeRegistry.init(alloc);
    ...
    // i64 is POD
    try std.testing.expectEqual(TypeCategory.pod, gen.classifyType(TypeRegistry.I64));

    // List is collection
    const list_type = try reg.makeList(TypeRegistry.I64);
    try std.testing.expectEqual(TypeCategory.collection, gen.classifyType(list_type));

    // Optional<i64> is POD (i64 is trivial)
    const opt_type = try reg.makeOptional(TypeRegistry.I64);
    try std.testing.expectEqual(TypeCategory.pod, gen.classifyType(opt_type));

    // Raw pointer is POD
    const ptr_type = try reg.makePointer(TypeRegistry.I64);
    try std.testing.expectEqual(TypeCategory.pod, gen.classifyType(ptr_type));
}

test "VWTGenerator getHashFnName" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    var reg = try TypeRegistry.init(alloc);
    ...
    // i64 should get i64_hash
    const i64_hash = gen.getHashFnName(TypeRegistry.I64, "i64");
    try std.testing.expect(i64_hash != null);
    try std.testing.expect(std.mem.eql(u8, i64_hash.?, "std.map.i64_hash"));

    // string should get string_hash
    const str_hash = gen.getHashFnName(TypeRegistry.STRING, "string");
    try std.testing.expect(str_hash != null);
    try std.testing.expect(std.mem.eql(u8, str_hash.?, "std.map.string_hash"));

    // f64 should get null (not hashable)
    const f64_hash = gen.getHashFnName(TypeRegistry.F64, "f64");
    try std.testing.expect(f64_hash == null);
}
