...
//!
//! Sits between the checker (type-checked AST) and the SSA builder. Every
//! expression, statement, and control-flow construct is lowered into IR nodes
//! inside basic blocks. The IR is function-scoped: each function has its own
//! set of locals, blocks, and nodes.
//!
//! Key types:
//!   Node       — a single IR operation (const, binary, call, branch, ...)
//!   Block      — a basic block containing an ordered list of nodes
//!   Local      — a named local variable or parameter
//!   Func       — a complete function (locals + blocks + nodes)
//!   FuncBuilder — incremental construction of a Func
...
/// Index into the node array of a function.
pub const NodeIndex = u32;

/// Sentinel value for an absent node.
pub const null_node: NodeIndex = std.math.maxInt(NodeIndex);

/// Index into the locals array of a function.
pub const LocalIdx = u32;

/// Sentinel value for an absent local.
pub const null_local: LocalIdx = std.math.maxInt(LocalIdx);

/// Index into the blocks array of a function.
pub const BlockIndex = u32;

/// Sentinel value for an absent block.
pub const null_block: BlockIndex = std.math.maxInt(BlockIndex);

/// Index into the params array of a function.
pub const ParamIdx = u32;

/// Index into the string literals table.
pub const StringIdx = u32;

/// Index into the globals array.
pub const GlobalIdx = u32;

/// Binary arithmetic, comparison, logical, bitwise, and atomic operations.
...
/// Unary operations: negation, bitwise complement, math intrinsics, atomics.
...
pub const SliceCap = struct { slice: NodeIndex };
...
pub const SliceHeader = struct { ptr: NodeIndex, len: NodeIndex, cap: NodeIndex };
...
pub const GcArrayNewData = struct { type_name: []const u8, data_idx: u32, offset: NodeIndex, length: NodeIndex };
...
/// A single IR operation. Every node has a type, source span, owning block,
/// and a tagged-union payload describing the operation.
pub const Node = struct {
    ...
    /// Tagged union of all IR operation kinds.
    pub const Data = union(enum) {
        ...
        gc_array_new_data: GcArrayNewData,
    ...
    pub fn init(data: Data, type_idx: TypeIndex, span: Span) Node {
        return .{ .type_idx = type_idx, .span = span, .block = null_block, .data = data };
    }
...
/// A basic block: a straight-line sequence of nodes ending in a terminator.
...
pub const Local = struct {
    ...
    alignment: u32 = 8,
    offset: i32 = 0,
    /// True when this local's type is a generic type parameter in a shared
    /// generic body. Address-only locals are never decomposed into components;
    /// all operations go through VWT witnesses or memcpy.
    ...
    overlap_group: u16 = 0,
    overlap_arm: u16 = 0,
    /// Parameter transfers ownership to callee. When true, the value is "sent"
    /// at the call site — caller must not use it after.
    is_sending: bool = false,

    pub fn init(name: []const u8, type_idx: TypeIndex, mutable: bool) Local {
        return .{ .name = name, .type_idx = type_idx, .mutable = mutable };
    }

    pub fn initParam(name: []const u8, type_idx: TypeIndex, param_idx: ParamIdx, size: u32) Local {
        return .{ .name = name, .type_idx = type_idx, .mutable = false, .is_param = true, .param_idx = param_idx, .size = size, .alignment = @min(size, 8) };
    }

    pub fn initWithSize(name: []const u8, type_idx: TypeIndex, mutable: bool, size: u32) Local {
        return .{ .name = name, .type_idx = type_idx, .mutable = mutable, .size = size, .alignment = @min(size, 8) };
    }
};

/// A complete, immutable function produced by FuncBuilder.build().
...
/// Incremental builder for constructing a single function's IR.
///
/// Manages locals (with lexical scoping via a shadow stack), basic blocks,
/// nodes, and string literals. Call build() to produce an immutable Func.
pub const FuncBuilder = struct {
    ...
    /// Set for impl methods named "deinit" — used by the driver to register
    /// ARC destructors without scanning function names.
    is_destructor: bool = false,
    is_export: bool = false,
    /// Async: local storing the TaskObject pointer (for lowerReturn to store result).
    async_task_local: ?LocalIdx = null,
    /// Async: the inner result type (T in Task(T)).
    async_result_type: ?TypeIndex = null,
    /// Async: true if this is a poll function (constructor/poll split).
    ...
    /// Add a sending parameter. The value is transferred to callee — caller
    /// must not use it after the call.
    ...
    /// Mark a local as address-only. Address-only locals are never decomposed
    /// by the SSA builder; all operations go through VWT witnesses or memcpy
    /// with runtime size.
    ...
    pub fn addStringLiteral(self: *FuncBuilder, str: []const u8) !StringIdx {
        for (self.string_literals.items, 0..) |existing, i| {
            if (std.mem.eql(u8, existing, str)) return @intCast(i);
        }
    ...
    /// Append a node to the current block, returning its index.
    ...
    pub fn emitSliceCap(self: *FuncBuilder, slice: NodeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .slice_cap = .{ .slice = slice } }, TypeRegistry.I64, span));
    }
    ...
    pub fn emitCall(self: *FuncBuilder, func_name: []const u8, args: []const NodeIndex, is_builtin: bool, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .call = .{
            .func_name = func_name,
            .args = try self.allocator.dupe(NodeIndex, args),
    ...
    pub fn emitCallIndirect(self: *FuncBuilder, callee: NodeIndex, args: []const NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .call_indirect = .{
            .callee = callee,
            .args = try self.allocator.dupe(NodeIndex, args),
        } }, type_idx, span));
    }
    ...
    pub fn emitClosureCall(self: *FuncBuilder, callee: NodeIndex, context: NodeIndex, args: []const NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .closure_call = .{
            .callee = callee,
            .context = context,
            .args = try self.allocator.dupe(NodeIndex, args),
        } }, type_idx, span));
    ...
    pub fn emitBranch(self: *FuncBuilder, condition: NodeIndex, then_block: BlockIndex, else_block: BlockIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .branch = .{
            .condition = condition,
            .then_block = then_block,
            .else_block = else_block,
        } }, TypeRegistry.VOID, span));
    }
    ...
    pub fn emitSelect(self: *FuncBuilder, condition: NodeIndex, then_value: NodeIndex, else_value: NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .select = .{
            .condition = condition,
    ...
    pub fn emitConvert(self: *FuncBuilder, operand: NodeIndex, from_type: TypeIndex, to_type: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .convert = .{
            .operand = operand,
            .from_type = from_type,
            .to_type = to_type,
        } }, to_type, span));
    ...
    pub fn emitTrap(self: *FuncBuilder, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .trap = {} }, TypeRegistry.VOID, span));
    }

    pub fn emitIndirectCall(self: *FuncBuilder, callee: NodeIndex, args: []const NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emitCallIndirect(callee, args, type_idx, span);
    }

    pub fn emitStoreFieldValue(self: *FuncBuilder, base: NodeIndex, field_idx: u32, offset: i64, value: NodeIndex, span: Span) !NodeIndex {
    ...
    pub fn emitPtrCast(self: *FuncBuilder, operand: NodeIndex, target_type: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .ptr_cast = .{ .operand = operand } }, target_type, span));
    }

    pub fn emitIntToPtr(self: *FuncBuilder, operand: NodeIndex, target_type: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .int_to_ptr = .{ .operand = operand } }, target_type, span));
    }

    pub fn emitPtrToInt(self: *FuncBuilder, operand: NodeIndex, target_type: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .ptr_to_int = .{ .operand = operand } }, target_type, span));
    }

    pub fn emitMakeSlice(self: *FuncBuilder, base_ptr: NodeIndex, start: ?NodeIndex, end: NodeIndex, elem_size: u32, type_idx: TypeIndex, span: Span) !NodeIndex {
    ...
    pub fn emitWasmGlobalRead(self: *FuncBuilder, global_idx: u32, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .wasm_global_read = .{ .global_idx = global_idx } }, type_idx, span));
    }
    ...
    pub fn emitGcStructNew(self: *FuncBuilder, type_name: []const u8, field_values: []const NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_struct_new = .{
            .type_name = type_name,
            .field_values = try self.allocator.dupe(NodeIndex, field_values),
        } }, type_idx, span));
    }
    ...
    pub fn emitGcStructGet(self: *FuncBuilder, base: NodeIndex, type_name: []const u8, field_idx: u32, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_struct_get = .{
            .base = base,
            .type_name = type_name,
            .field_idx = field_idx,
        } }, type_idx, span));
    ...
    pub fn emitGcStructSet(self: *FuncBuilder, base: NodeIndex, type_name: []const u8, field_idx: u32, value: NodeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_struct_set = .{
            .base = base,
            .type_name = type_name,
            .field_idx = field_idx,
            .value = value,
    ...
    pub fn emitGcArrayNew(self: *FuncBuilder, type_name: []const u8, init_val: NodeIndex, length: NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_array_new = .{
            .type_name = type_name,
            .init_val = init_val,
            .length = length,
        } }, type_idx, span));
    ...
    pub fn emitGcArrayNewDefault(self: *FuncBuilder, type_name: []const u8, length: NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_array_new_default = .{
            .type_name = type_name,
            .length = length,
        } }, type_idx, span));
    }
    ...
    pub fn emitGcArrayNewFixed(self: *FuncBuilder, type_name: []const u8, values: []const NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_array_new_fixed = .{
            .type_name = type_name,
            .values = try self.allocator.dupe(NodeIndex, values),
        } }, type_idx, span));
    }

    pub fn emitGcArrayNewData(self: *FuncBuilder, type_name: []const u8, data_idx: u32, offset: NodeIndex, length: NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_array_new_data = .{
            .type_name = type_name,
            .data_idx = data_idx,
            .offset = offset,
            .length = length,
    ...
    pub fn emitGcArrayGet(self: *FuncBuilder, type_name: []const u8, array: NodeIndex, index: NodeIndex, type_idx: TypeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_array_get = .{
            .type_name = type_name,
            .array = array,
            .index = index,
        } }, type_idx, span));
    ...
    pub fn emitGcArraySet(self: *FuncBuilder, type_name: []const u8, array: NodeIndex, index: NodeIndex, value: NodeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_array_set = .{
            .type_name = type_name,
            .array = array,
            .index = index,
            .value = value,
    ...
    pub fn emitGcArrayCopy(self: *FuncBuilder, dst_type_name: []const u8, src_type_name: []const u8, dst: NodeIndex, dst_offset: NodeIndex, src: NodeIndex, src_offset: NodeIndex, length: NodeIndex, span: Span) !NodeIndex {
        return self.emit(Node.init(.{ .gc_array_copy = .{
    ...
    /// Finalize the function: compute frame layout and produce an immutable Func.
    pub fn build(self: *FuncBuilder) !Func {
        var params = std.ArrayListUnmanaged(Local){};
        defer params.deinit(self.allocator);
        for (self.locals.items) |local| {
            if (local.is_param) try params.append(self.allocator, local);
        }
...
pub const Global = struct {
    ...
    pub fn init(name: []const u8, type_idx: TypeIndex, is_const: bool, span: Span) Global {
        return .{ .name = name, .type_idx = type_idx, .is_const = is_const, .span = span };
    }

    pub fn initWithSize(name: []const u8, type_idx: TypeIndex, is_const: bool, span: Span, size: u32) Global {
        return .{ .name = name, .type_idx = type_idx, .is_const = is_const, .span = span, .size = size };
    }
};

/// A struct definition registered in the IR.
pub const StructDef = struct { name: []const u8, type_idx: TypeIndex, span: Span };

/// The complete intermediate representation for a compilation unit:
/// all functions, globals, struct definitions, and the shared type registry.
pub const IR = struct {
    funcs: []const Func = &.{},
    ...
    pub fn init(allocator: Allocator, type_reg: *TypeRegistry) IR {
        return .{ .types = type_reg, .allocator = allocator };
    }
...
/// Top-level IR builder managing multiple functions, globals, and structs.
///
/// Supports nested function declarations (closures) via a saved-function stack.
/// Forward declarations allow call sites to reference functions before their
/// body is emitted.
pub const Builder = struct {
    ...
    pub fn init(allocator: Allocator, type_reg: *TypeRegistry) Builder {
        return .{ .ir = IR.init(allocator, type_reg), .allocator = allocator };
    }
    ...
    pub fn func(self: *Builder) ?*FuncBuilder {
        return if (self.current_func) |*fb| fb else null;
    }
    ...
    /// Forward-declare a function name so call sites can reference it before
    /// the body is emitted. Does NOT make hasFunc() return true.
    ...
    pub fn lookupGlobal(self: *const Builder, name: []const u8) ?struct { idx: GlobalIdx, global: Global } {
        for (self.globals.items, 0..) |g, idx| {
            if (std.mem.eql(u8, g.name, name)) return .{ .idx = @intCast(idx), .global = g };
        }
        return null;
    }
...
/// Write a human-readable description of a node to the given writer.
...
test "binary op predicates" {
    ...
    try std.testing.expect(!BinaryOp.shl.isLogical());
    try std.testing.expect(!BinaryOp.@"and".isBitwise());
}
...
test "node properties" {
    ...
    const jump_node = Node.init(.{ .jump = .{ .target = 1 } }, TypeRegistry.VOID, Span.fromPos(Pos.zero));
    try std.testing.expect(jump_node.isTerminator());

    const trap_node = Node.init(.{ .trap = {} }, TypeRegistry.VOID, Span.fromPos(Pos.zero));
    try std.testing.expect(trap_node.isTerminator());
    try std.testing.expect(!trap_node.isConstant());
}

test "FuncBuilder init emit build cycle" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    const alloc = arena.allocator();

    var fb = FuncBuilder.init(alloc, "test_fn", TypeRegistry.VOID, TypeRegistry.INT, Span.fromPos(Pos.zero));

    // Entry block (0) is created automatically
    try std.testing.expectEqual(@as(BlockIndex, 0), fb.currentBlock());

    // Add a local and a param
    const local_idx = try fb.addLocal(Local.init("x", TypeRegistry.INT, true));
    try std.testing.expectEqual(@as(LocalIdx, 0), local_idx);
    const param_idx = try fb.addParam("y", TypeRegistry.INT, 8);
    try std.testing.expectEqual(@as(LocalIdx, 1), param_idx);

    // Lookup
    try std.testing.expectEqual(local_idx, fb.lookupLocal("x").?);
    try std.testing.expectEqual(param_idx, fb.lookupLocal("y").?);
    try std.testing.expect(fb.lookupLocal("z") == null);

    // Emit some nodes
    const c = try fb.emitConstInt(42, TypeRegistry.INT, Span.fromPos(Pos.zero));
    try std.testing.expectEqual(@as(NodeIndex, 0), c);
    _ = try fb.emitStoreLocal(local_idx, c, Span.fromPos(Pos.zero));

    // New block
    const blk = try fb.newBlock("then");
    try std.testing.expectEqual(@as(BlockIndex, 1), blk);
    fb.setBlock(blk);
    _ = try fb.emitRet(c, Span.fromPos(Pos.zero));

    // Build
    const f = try fb.build();
    try std.testing.expectEqual(@as(usize, 2), f.locals.len);
    try std.testing.expectEqual(@as(usize, 1), f.params.len);
    try std.testing.expectEqual(@as(usize, 3), f.nodes.len);
    try std.testing.expectEqual(@as(usize, 2), f.blocks.len);
}

test "FuncBuilder scope save and restore" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    const alloc = arena.allocator();

    var fb = FuncBuilder.init(alloc, "scope_test", TypeRegistry.VOID, TypeRegistry.VOID, Span.fromPos(Pos.zero));

    _ = try fb.addLocalWithSize("x", TypeRegistry.INT, true, 8);
    const mark = fb.markScopeEntry();

    // Inner scope shadows x
    _ = try fb.addLocalWithSize("x", TypeRegistry.INT, true, 8);
    try std.testing.expectEqual(@as(LocalIdx, 1), fb.lookupLocal("x").?);

    fb.restoreScope(mark);
    try std.testing.expectEqual(@as(LocalIdx, 0), fb.lookupLocal("x").?);
}

test "FuncBuilder needsTerminator" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    const alloc = arena.allocator();

    var fb = FuncBuilder.init(alloc, "term_test", TypeRegistry.VOID, TypeRegistry.VOID, Span.fromPos(Pos.zero));

    // Empty block needs a terminator
    try std.testing.expect(fb.needsTerminator());

    // After emitting a return, it does not
    _ = try fb.emitRet(null, Span.fromPos(Pos.zero));
    try std.testing.expect(!fb.needsTerminator());

    // New block needs one again
    const blk = try fb.newBlock("next");
    fb.setBlock(blk);
    try std.testing.expect(fb.needsTerminator());
}
...
test "string literal deduplication" {
    ...
    var fb = FuncBuilder.init(alloc, "str_test", TypeRegistry.VOID, TypeRegistry.VOID, Span.fromPos(Pos.zero));

    const idx1 = try fb.addStringLiteral("hello");
    const idx2 = try fb.addStringLiteral("world");
    const idx3 = try fb.addStringLiteral("hello");
    try std.testing.expectEqual(idx1, idx3);
    try std.testing.expect(idx1 != idx2);
}

test "IR builder with globals" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    ...
    builder.startFunc("main", TypeRegistry.VOID, TypeRegistry.INT, Span.fromPos(Pos.zero));
    if (builder.func()) |fb| {
        _ = try fb.emitConstInt(0, TypeRegistry.INT, Span.fromPos(Pos.zero));
        _ = try fb.emitRet(0, Span.fromPos(Pos.zero));
    }
    try builder.endFunc();

    try std.testing.expect(builder.hasFunc("main"));
    try std.testing.expect(!builder.hasFunc("other"));

    try builder.declareFunc("forward");
    try std.testing.expect(!builder.hasFunc("forward"));
    try std.testing.expect(builder.hasDeclaredFunc("forward"));
    try std.testing.expect(builder.hasDeclaredFunc("main"));
    ...
    try std.testing.expectEqual(@as(GlobalIdx, 0), lookup.?.idx);
    ...
    try std.testing.expect(ir.getFunc("missing") == null);
    try std.testing.expect(ir.getGlobal("counter") != null);
    try std.testing.expect(ir.getGlobal("missing") == null);
}
...
test "local variable layout" {
    ...
    try std.testing.expect(!local.is_address_only);
...
test "debugPrintNode" {
    ...
    const node = Node.init(.{ .const_int = .{ .value = 99 } }, TypeRegistry.INT, Span.fromPos(Pos.zero));
    try debugPrintNode(&node, writer);
    const written = fbs.getWritten();
    try std.testing.expect(std.mem.indexOf(u8, written, "const_int 99") != null);
}
