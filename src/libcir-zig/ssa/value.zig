...
//!
//! Each Value has an Op (what it computes), a type, arguments (other Values),
//! and auxiliary data. Values live inside Blocks and are connected via use
//! chains for dead-code elimination and register allocation.
...
/// Symbol with offset for address computations.
pub const SymbolOff = struct { sym: ?*anyopaque = null, offset: i64 = 0 };

/// Tagged union of auxiliary data carried by a Value.
...
/// Physical location assigned by register allocation.
...
test "Value creation" {
    var v = Value.init(1, .const_int, @enumFromInt(0), null, .{});
    v.aux_int = 42;
...
test "Value use count tracking" {
    var v1 = Value.init(1, .const_int, @enumFromInt(0), null, .{});
    var v2 = Value.init(2, .const_int, @enumFromInt(0), null, .{});
    var v3 = Value.init(3, .add, @enumFromInt(0), null, .{});
    v3.addArg(&v1);
    v3.addArg(&v2);
...
test "Value isConst" {
    var v1 = Value.init(1, .const_int, @enumFromInt(0), null, .{});
    var v2 = Value.init(2, .add, @enumFromInt(0), null, .{});
...
test "Value dynamic argument allocation" {
    ...
    for (&args, 0..) |*arg, i| arg.* = Value.init(@intCast(i + 1), .const_int, @enumFromInt(0), null, .{});

    var phi = Value.init(10, .phi, @enumFromInt(0), null, .{});
