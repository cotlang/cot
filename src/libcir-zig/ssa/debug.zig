...
//!
//! Provides multiple output formats for SSA functions (text, DOT, HTML),
//! liveness analysis for dead-code marking, phase snapshots for change
//! tracking, and verification of SSA invariants after each pass.

const std = @import("std");
const foundation = @import("foundation");
const TypeRegistry = foundation.types.TypeRegistry;
const pipeline_debug = foundation.debug;
...
/// Output format for dump functions.
...
/// Dump a function in text or DOT format.
...
/// Dump a function as human-readable text.
...
fn dumpValue(v: *const Value, writer: anytype) !void {
    ...
    switch (v.aux) {
        ...
        .type_ref => |t| try writer.print(" type({d})", .{@intFromEnum(t)}),
...
/// Dump a function as a DOT graph for Graphviz.
...
pub fn verify(f: *const Func, allocator: std.mem.Allocator) ![]const []const u8 {
    ...
    for (f.blocks.items) |b| {
        for (b.succs) |succ_edge| {
            if (!block_set.contains(succ_edge.b.id)) {
                try errors.append(allocator, try std.fmt.allocPrint(allocator, "b{d}: successor b{d} not in function", .{ b.id, succ_edge.b.id }));
            }
        }
        for (b.preds) |pred| {
        ...
        for (b.values.items) |v| {
            ...
            if (v.uses < 0) {
                ...
                var arg_ids_buf: [128]u8 = undefined;
                var arg_pos: usize = 0;
                for (v.args) |arg| {
                    if (arg_pos > 0 and arg_pos < arg_ids_buf.len - 5) {
                        arg_ids_buf[arg_pos] = ',';
                        arg_pos += 1;
                    }
                    const written = std.fmt.bufPrint(arg_ids_buf[arg_pos..], "v{d}", .{arg.id}) catch break;
                    arg_pos += written.len;
                }
        ...
        for (b.succs, 0..) |succ_edge, i| {
            if (succ_edge.i >= succ_edge.b.preds.len or succ_edge.b.preds[succ_edge.i].b != b) {
...
/// Run verification after a pass. If errors found, log them.
...
/// Free error strings from verify().
...
/// Snapshot of a value's state at a particular compilation phase.
pub const ValueSnapshot = struct { id: ID, op: Op, arg_ids: []ID, uses: i32, aux_int: i64 };

/// Snapshot of a block's state at a particular compilation phase.
pub const BlockSnapshot = struct { id: ID, kind: @import("block.zig").BlockKind, values: []ValueSnapshot, succ_ids: []ID };

/// Snapshot of the entire function at a particular compilation phase.
pub const PhaseSnapshot = struct {
    ...
    pub fn capture(allocator: std.mem.Allocator, f: *const Func, name: []const u8) !PhaseSnapshot {
        ...
        for (f.blocks.items, 0..) |b, i| {
            ...
            for (b.succs, 0..) |succ_edge, k| succ_ids[k] = succ_edge.b.id;
...
/// Statistics about changes between two phase snapshots.
...
test "verify catches edge invariant violations" {
    ...
    const exit_b = try f.newBlock(.ret);
    try entry.addEdgeTo(allocator, exit_b);

    const errs = try verify(&f, allocator);
    defer freeErrors(errs, allocator);
