...
//!
//! A Block is a straight-line sequence of Values with a single entry point
//! and a terminator (branch, return, etc.). Blocks are connected via
//! predecessor/successor edges to form the control flow graph.
...
/// The kind of terminator at the end of a block.
...
/// A directed edge in the CFG, recording the block and the index in the
/// peer's edge list for O(1) bidirectional updates.
...
/// Branch prediction hint for profile-guided optimization.
...
test "Block control values" {
    ...
    defer b.deinit(allocator);
    var v = Value.init(1, .const_bool, @enumFromInt(0), &b, .{});
    b.setControl(&v);
