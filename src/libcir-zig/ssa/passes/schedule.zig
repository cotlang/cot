...
fn getScore(v: *Value, is_control: bool) Score {
    ...
    if (v.type_idx == TypeRegistry.SSA_MEM or v.writesMemory()) return .memory;
    return switch (v.op) {
        .phi => .phi,
        .arg => .arg,
        .init_mem => .init_mem,
...
fn scheduleBlock(allocator: std.mem.Allocator, block: *Block, f: *Func) !void {
    ...
    for (values) |v| {
        ...
        const is_mem_producer = v.type_idx == TypeRegistry.SSA_MEM or v.writesMemory();
        if (is_mem_producer) {
...
test "getScore returns correct priorities" {
    ...
    const phi = try f.newValue(.phi, @enumFromInt(0), b, .{});
    try b.addValue(allocator, phi);
    const arg = try f.newValue(.arg, @enumFromInt(0), b, .{});
    try b.addValue(allocator, arg);
    const add = try f.newValue(.add, @enumFromInt(0), b, .{});
    try b.addValue(allocator, add);
    const store = try f.newValue(.store, TypeRegistry.SSA_MEM, b, .{});
    ...
    try testing.expectEqual(Score.control, getScore(add, true));
}
