...
//!
//! Every SSA Value carries an Op that defines the computation. Ops range
//! from platform-independent generic operations (add, sub, load, store)
//! to machine-specific lowered forms (arm64_add, wasm_i64_add).
...
/// Metadata describing an SSA operation's properties.
...
/// Register constraint info for register allocation.
...
/// Auxiliary data type carried by an operation.
...
const op_info_table = blk: {
    ...
    table[@intFromEnum(Op.slice_make)] = .{ .name = "SliceMake", .arg_len = 3 };
    table[@intFromEnum(Op.string_concat)] = .{ .name = "StringConcat", .arg_len = 2, .has_side_effects = true, .call = true };
    table[@intFromEnum(Op.opt_make)] = .{ .name = "OptMake", .arg_len = 2 };
